...
IF @@rowcount > 0 and Update(PrepaidTip)
	Begin
		...
		SELECT		I.CustomerID, GETDATE(), D.PrepaidTip, NewPrepaidTip
		FROM inserted I
				INNER JOIN deleted D ON D.PrePaidTip = I.PrePaidTip
	End
return
...
if @@rowcount>0 and update(CarrierId)
	...
	If exists (	Select * 
				from inserted 
						inner join Carrier on inserted.CarrierID  = Carrier.CarrierID 
				where Active = 'N')
		Begin
